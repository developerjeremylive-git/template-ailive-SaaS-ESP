...
var defineWebSocketHelper = /* @__PURE__ */ __name((handler) => {
  return (createEvents, options) => {
    return /* @__PURE__ */ __name(async function UpgradeWebSocket(c, next) {
...
app.use("/*", async (c, next) => {
  try {
    ...
    if (response.status === 404 && !path.includes(".")) {
      console.log(`File not found: ${path}, serving index.html`);
      return module({
        root: "./",
    ...
    console.error("Static file serving error:", {
      error: error.message,
      stack: error.stack,
